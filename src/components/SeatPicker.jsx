...
export default function SeatPicker({ route, user, onDone, onError, onSuccess }) {
  ...
  // Ao chegar no passo de embarque, já tentamos usar a localização atual do
  // passageiro como ponto de partida — ele pode arrastar o marcador ou
  // digitar outro endereço se estiver pegando a carona em outro lugar.
  useEffect(() => {
    if (step === 'pickup' && !pickupCoords && !locating) {
      useMyLocation()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [step])

  async function handleMapMove(lat, lng) {
  ...
  return (
    <div>
      ...
      <div className="seat-legend">
        ...
        <span><span className="swatch" style={{ background: 'var(--line-700)', border: '1.5px solid var(--line-600)' }} /> Ocupado</span>
      </div>
      <button
        className="btn btn-primary btn-block"
