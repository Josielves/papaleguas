...
export default function SeatPicker({ route, user, onDone, onError, onSuccess }) {
  ...
  const [step, setStep] = useState('seats') // 'seats' | 'pickup' | 'confirm'
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', gap: '0.625rem' }}>
          <button className="btn btn-secondary btn-block" onClick={() => setStep('pickup')} disabled={confirming}>
            Voltar
          </button>
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', gap: '0.625rem' }}>
          ...
          <button className="btn btn-primary btn-block" onClick={() => setStep('confirm')}>Continuar</button>
        </div>
      </div>
    )
  }
